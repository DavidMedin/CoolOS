...
debug:
    qemu-system-i386 -hda zig-out/bin/coolos.img -s -S -no-reboot -no-shutdown # -d int <- if you want too much info about an interrupt.

# Run gdb and qemu
debug-gdb: build
    kitty --detach --directory=. gdb -ex 'target remote localhost:1234' --symbols=zig-out/bin/coolos.bin
    qemu-system-i386 -hda zig-out/bin/coolos.img -s -S -no-reboot -no-shutdown &
